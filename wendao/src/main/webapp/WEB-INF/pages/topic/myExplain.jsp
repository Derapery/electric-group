...
<div>
    ...
    <div class="list-conter">
        ...
        <main class="topics-container gao-du">
            <c:forEach items="${explains}" var="explain" varStatus="x">
                <div class="topic-all">
                    <div class="topic-list-detail-head">
                        ...
                        <a class="athor-name" href="${ctxPath}/customer/mine?id=${explain.topic.author.id}">${explain.author.nickname}</a>
                        <span class="athor-where">发布于${explain.topic.publishTime}</span>
                        <c:if test="${explain.topic.author.concern ==0}">
                            <a class="guanzhu :hover"  href="${ctxPath}/customer/fansAction?customer_id=${explain.topic.author.id}&concern=0&size=${paging.size}&current=${paging.current}">+关注</a>
                        </c:if>
                        <c:if test="${explain.topic.author.concern ==1}">
                            <a class="guanzhu :hover" style="height: 29px;width: 91px" href="${ctxPath}/customer/fansAction?customer_id=${explain.topic.author.id}&concern=1&size=${paging.size}&current=${paging.current}">取消关注</a>
                        </c:if>
                    </div>
                    <div class="content-a">
                        <a href="${ctxPath}/topic/detail?id=${explain.topic.id}">
                            <div class="title-content">
                                <h3 class="topic-detail-title">${explain.topic.title}</h3>
                            </div>
                        </a>
                        <div class="topic-content">${explain.topic.content}</div>
                        <div class="category-content">
                            <h6 class="topic-detail-category">#${explain.topic.category_name}</h6>
                        </div>
                    </div>

                    <div class="footerr">
                        <span class="col-6  row justify-content-end">
                            <c:if test="${ explain.topic.state==null}">
                            <span class="col-2 justify-content-start">
                             <a href="${ctxPath}/topic/thumbsState?praise=1&size=${paging.size}&current=${paging.current} &topic_id=${topic.id}" class="praise">
                                       <i class="fa fa-thumbs-o-up"></i>(${explain.topic.thumbUpCount})
                            </a>
                             </span>
                             ...
                             <a href="${ctxPath}/topic/thumbsState?praise=0&size=${paging.size}&current=${paging.current} &topic_id=${topic.id}" class="praise">
                                   <i class="fa fa-thumbs-o-down"></i>(${explain.topic.thumbDownCount})
                              </a>
                              </span>
                            </c:if>
                         <c:if test="${ explain.topic.state == 1}">
                             <span class="col-2 justify-content-start">
                                <a href="${ctxPath}/topic/thumbsState?praise=1&size=${paging.size}&current=${paging.current} &topic_id=${topic.id}" class="praise">
                                   <i class="fa fa-thumbs-up"></i>(${explain.topic.thumbUpCount})
                               </a>
                              </span>
                             <span class="col-2 offset-3">
                         ...
                         <c:if test="${ explain.topic.state == 0}">
                                <span class="col-2 justify-content-start">
                             ...
                             <span class="col-2 offset-2">
                                    <a href="${ctxPath}/topic/thumbsState?praise=0&size=${paging.size}&current=${paging.current} &topic_id=${topic.id}" class="praise">
                                       <i class="fa fa-thumbs-down "></i>(${explain.topic.thumbDownCount})
                                    </a>
                               </span>
                         </c:if>
                            <span class="col-3 offset-1 ">
                                <a class="fa fa-eye" href="#"></a> ${explain.topic.priority}
                            </span>
                            <span class="col-2 offset-1 " style="padding-right: 1px">
                                <a class="fa fa-commenting-o" href="${ctxPath}/topic/publishExplain?id=${explain.topic.id}" title="我来解答"> </a> ${explain.topic.explainCount}
                            </span>
                            <c:if test="${explain.topic.author.id == customer.id}">
                ...
                <div class="topic-all" style="height:160px">
                    <div class="content-a">
                        <div class="topic-content">${explain.content}</div>
                    </div>
                    <div class="footerr">
                        <span class="col-5  offset-7 row justify-content-end">
                            <c:if test="${ explain.topic.state==null}">
                            <span class="col-2 offset-3 justify-content-start">
                             <a href="${ctxPath}/topic/thumbsState?praise=1&size=${paging.size}&current=${paging.current} &topic_id=${topic.id}" class="praise">
                                       <i class="fa fa-thumbs-o-up"></i>(${explain.praise})
                            </a>
                             </span>
                                <span  class="col-2 offset-2">
                             <a href="${ctxPath}/topic/thumbsState?praise=0&size=${paging.size}&current=${paging.current} &topic_id=${topic.id}" class="praise">
                                   <i class="fa fa-thumbs-o-down"></i>(${explain.despise})
                              </a>
                              </span>
                            </c:if>
                            <span class="col-3 offset-1">
                                 <a href="${ctxPath}/explain/delete?id=${explain.id}&size=${paging.size}&current=${paging.current} ">删除评论</a>
                            </span>
                        </span>
                    </div>
                </div>
